...
class JobWorkflowApp(QWidget):
    def __init__(self):
        ...
        self.instructions_and_result = QTextEdit()
        self.instructions_and_result.setReadOnly(True)
        self.instructions_and_result.setStyleSheet("font-size: 16pt; padding: 10px;")
        self.instructions_and_result.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.result_indicator.setStyleSheet(
            "font-size: 18pt; font-weight: bold; background-color: lightgray; padding: 10px; border-radius: 8px;"
        )
        self.left_layout.addWidget(self.instructions_and_result, stretch=2)
        self.left_layout.addWidget(self.result_indicator, stretch=1)
        ...
        self.job_details.setStyleSheet("font-size: 14pt; " \
        "padding: 10px;")
    ...
    def send_next_task(self):
        ...
        self.instructions_and_result.setHtml(f"<b>Instructions:</b><br>{task_instructions[task]}")
    ...
    def check_ack(self):
        ...
        if acknowledgements[task]:
            ...
            if task == "job_allocation":
                ...
                details_text = (
                    f"<b>Part Details:</b><br>"
                    f"SL NO: {data.get('SL NO', '')}<br>"
                    f"Part Name: {data.get('PART NAME', '')}<br>"
                    f"Part Number: {data.get('PART NUMBER', '')}<br>"
                    f"Model: {data.get('MODEL', '')}<br>"
                    f"Part Weight: {data.get('PART WEIGHT', '')}<br>"
                    f"Bin & Cover Weight: {data.get('BIN & COVER WEIGHT', '')}<br>"
                    f"Cover Qty: {data.get('COVER QTY', '')}<br>"
                    f"Bin Qty: {data.get('BIN QTY', '')}<br>"
                    f"Bin Weight: {data.get('BIN WEIGHT', '')}<br>"
                    f"Cover Qty Variation: {data.get('COVR QTY VARIATION', '')}"
                )
                self.job_details.setHtml(details_text)

            self.current_task_index += 1
            self.send_next_task()# Inside check_ack(), after job_allocation block
            if task == "job_closeout":
                data = current_job_log.get(task, {})
                if data.get("count_ok", False):
                    self.result_indicator.setText("PASS ✅")
                    self.result_indicator.setStyleSheet(
                        "font-size: 18pt; font-weight: bold; background-color: green; color: white; padding: 10px; border-radius: 8px;"
                    )
                else:
                    self.result_indicator.setText("FAIL ❌")
                    self.result_indicator.setStyleSheet(
                        "font-size: 18pt; font-weight: bold; background-color: red; color: white; padding: 10px; border-radius: 8px;"
                    )
    ...
    def emergency_stop(self):
        ...
        self.instructions_and_result.setText("System halted immediately. Resolve issues and restart.")
